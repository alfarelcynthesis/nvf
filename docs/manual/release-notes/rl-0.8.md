...
- Upgrade [obsidian.nvim] to use a maintained fork, instead of the unmaintained
  ...
  - Commands like `ObsidianBacklinks` are now merged into `Obisidian backlinks`,
    still supported by default.
  - Integration with `snacks.image` for rendering in-workspace assets added.
